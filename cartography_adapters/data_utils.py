...
def convert_string_to_unique_number(string: str) -> int:
    ...
    id_map = {'e': '0', 'c': '1', 'n': '2'}
    # SNLI-specific hacks.
    if string.startswith('vg_len'): code='555'
    elif string.startswith('vg_verb'): code='444'
    else: code='000'
    try: number = int(code + re.sub(r"\D", "", string) + id_map.get(string[-1], '3'))
    except:
...
def read_data(file_path: str, task_name: str,
              guid_as_int: bool = False):
...
#     `guid_index`: should be 2 for SNLI, 0 for MNLI and None for any random tsv file.
    if task_name == "MNLI":
        return read_glue_tsv(file_path, guid_index=0, guid_as_int=guid_as_int)
    elif task_name == "SNLI":
        return read_glue_tsv(file_path, guid_index=2, guid_as_int=guid_as_int)
...
def read_jsonl(file_path: str, key: str = "pairID"):
    '''
    Reads JSONL file to recover mapping between one particular key field in the line and the result of the line as a JSON dict. 
...
class DataReader:
    '''class to read '''
    def __init__(self, path: Union[str, Path], task_name="MNLI"):
        pass
